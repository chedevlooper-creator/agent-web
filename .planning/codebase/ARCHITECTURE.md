...
| Zustand Store | Client state: sessions, messages, UI prefs (optimistic updates) | `apps/web/lib/store.ts` |
...
| Skills Route | GET: list installed skills from SKILL.md files | `apps/web/app/api/skills/route.ts` |
...
- **Layered separation**: UI → Zustand store → API routes → server DB layer → ORM/DB
- **Streaming-first**: Chat responses use AI SDK v4 `toDataStreamResponse()`, manual client-side stream parsing
- **Optimistic updates**: Zustand mutations apply locally first, persist to DB async, rollback on failure
- **Multi-provider LLM**: Abstraction over OpenAI, OpenRouter, DeepSeek via `createOpenAI()`
- **Auth**: Cookie-based session tokens with bcrypt password hashing, enforced via `getUserIdFromRequest()` middleware on all DB routes

## Layers

**Presentation Layer (Client-side React):**
- Purpose: Renders chat UI, handles user input, manages local state
...
- Key abstractions: Optimistic UI via `appendLocalMessage` / `patchLocalMessage`

**Client State Layer (Zustand):**
- Purpose: Central client-side state with optimistic update patterns
- Location: `apps/web/lib/store.ts`
- Contains: Session/message lists, UI prefs, provider config, skill toggles
...
- Depends on: `apps/web/lib/db.ts`, `apps/web/lib/auth.ts`, `packages/core`
- Patterns: Zod schema validation on every mutating route

**Server DB Layer:**
- Purpose: Business logic for all DB operations
- Location: `apps/web/lib/db.ts`
- Contains: CRUD for projects, sessions, messages, memories, API keys, Obsidian config
...
**Data Package Layer:**
- Purpose: DB schema defs, client singleton, migration runner
...
- Contains: Tool defs using `ai/tool` + `zod`, context compression utils, TypeScript types
...
4. **Stream request** via `streamChat()` → `fetch(POST /api/chat)` with JSON `{ messages, provider, model, enabledSkills, files }`
5. **Server side** (`/api/chat` route):
   - Validates API key from env
   - Creates LLM client via `createOpenAI()` (provider-specific: OpenRouter custom baseURL, DeepSeek fetch wrapper to disable thinking)
...
7. **Real-time UI updates**: `patchLocalMessage()` batches via `requestAnimationFrame` to avoid excessive re-renders
8. **Post-stream persistence**: After stream completes, final text persisted to DB via `persistMessage()`
...
1. LLM decides to use tool → AI SDK handles server-side execution
2. Tool implementations in `packages/core/src/tools/` receive args, execute, return results
3. Results stream back through data stream as tool result chunks
4. Client updates tool call bubbles in real-time: running → completed
...
- **localStorage**: Only UI prefs (sidebar state, provider, model) under `"agent-web-ui-prefs"`
- **Server (SQLite)**: All sessions and messages survive page reloads via libsql/Drizzle ORM
- **Stream state**: Local-only insert (`appendLocalMessage`) + batched update (`patchLocalMessage`) for streaming, persisted after stream completes

## Key Abstractions

**Tool System:**
- Purpose: LLM executable capabilities
...
- Pattern: Each tool defined with `tool()` from `ai` SDK, Zod param schema, `execute` function
- Backend abstraction: Terminal and execute-code support `local` or `docker` via `TERMINAL_BACKEND` env var
- Path security: File tools use `resolveSafePath()` from `packages/core/src/tools/path-security.ts`

**LLM Provider Abstraction:**
- Purpose: Unifies multiple LLM providers under single interface
...
- Purpose: Immediate UI responsiveness with async persistence
...
- Pattern: Char-based token estimation (char/4), sliding window preserves system prompt + first user message + most recent
...
- Responsibilities: Shell layout, sidebar + chat interface init
...
- **Node externals:** `@libsql/client`, `pdf-parse`, `mammoth`, `xlsx` marked as `serverExternalPackages` in `next.config.ts` — native/Node-only modules
- **Transpilation:** `@agent-web/core` and `@agent-web/db` are `transpilePackages` — tsc-built, handled by Next.js bundler

## Anti-Patterns

### Duplicate Provider Branch

**What happens:** `/api/chat` route has dead branch — `provider === "deepseek"` appears twice (lines 146 and 165 in route.ts), second branch unreachable.
**Why wrong:** Dead code adds confusion, suggests incomplete refactor.
**Fix:** Remove second `else if (provider === "deepseek")` block or consolidate.

### Inline Stream Parser Duplicates AI SDK Logic

**What happens:** Client manually parses Vercel AI SDK data stream format (`0:`, `1:`, `2:`, `9:`, `a:` prefixes) instead of using `useChat` from `ai/react`.
**Why wrong:** Custom parser must maintain compatibility with multiple stream format versions, may miss SDK edge cases.
**Fix:** Accepted trade-off — chosen deliberately for full control. Adds maintenance burden that should be documented.

## Error Handling

**Strategy:** Try/catch at API route boundaries, JSON error messages with appropriate HTTP status codes

**Patterns:**
- API routes wrap handler logic in try/catch, return `{ error: message }` with 400/401/500 status
- Chat streaming errors embedded in data stream as `3:` error chunks
...
**Logging:** `console.error` for server errors; toast for client user-visible errors
**Validation:** Zod schemas on all mutating API routes
**Auth:** Cookie-based `session_token` validated via `getUserIdFromRequest()` for all DB-touching routes; public routes (chat, config status) use server env vars directly
**Rate Limiting:** In-memory sliding window in `apps/web/lib/rate-limit.ts` (default: 100 req per 60s window per IP)
