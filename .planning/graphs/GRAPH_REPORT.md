...
- Verdict: large enough for graph structure to add value.
...
- Run `git rev-parse HEAD` and compare to check staleness.
...
## Knowledge Gaps
- **3 thin communities (<3 nodes) omitted** — run `graphify query` to explore.

## Suggested Questions
- **Why does `genId()` connect Community 5 to Community 0?**
  _High betweenness centrality (0.197) — cross-community bridge._
- **Why does `cn()` connect Community 0 to Community 5, Community 6?**
  _High betweenness centrality (0.168) — cross-community bridge._
- **Why does `getUserIdFromRequest()` connect Community 2 to Community 1, Community 4, Community 5?**
  _High betweenness centrality (0.141) — cross-community bridge._
- **Are 37 inferred relationships involving `getDb()` actually correct?**
  _37 INFERRED edges — need verification._
- **Are 22 inferred relationships involving `getUserIdFromRequest()` actually correct?**
  _22 INFERRED edges — need verification._
- **Are 4 inferred relationships involving `validateSession()` actually correct?**
  _4 INFERRED edges — need verification._
- **Should Community 0 be split into smaller modules?**
  _Cohesion 0.05 — nodes weakly interconnected._
